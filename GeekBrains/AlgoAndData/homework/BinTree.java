/*package GeekBrains.AlgoAndData.homework;
...
public class BinTree<T extends Comparable<T>> {
    ...
    public boolean add(T value) { // добавление, отдаем значение
        if (root == null) {      // если головы нет
            Node newNode = new Node(value);
            root = newNode; // новую  ноду делаем головой
            root.color = Color.Black; // root всегда черный
            return true;
        }
        if (addNode(root, value) != null) // если голова есть, то заполн дерево по addNode
            return true;
        return false;
    }

    private Node addNode(Node node, T value) { // добавление, отдаем ноду и значение
        if (node.value.compareTo(value) == 0) // если значения равны, то
            return null;
    ...
    private boolean contain(T value) {
        ...
        while (currentNode != null) {
            if (currentNode.value.equals(value)) // если нашли наше значение 
                return true;                     // возвращаем true
            if (currentNode.value.compareTo(value) > 0)
                currentNode = currentNode.left;     // если значение тек ноды больше нашего, то уходим налево
            else
                currentNode = currentNode.right;    // иначе уходим направо
        }
    ...
    private void colorSwap(Node node) { // отдаем текущ ноду
        node.right.color = Color.Black; // правый ребенок - на черный
        node.left.color = Color.Black;  // левый ребенок - на черный
        node.color = Color.Red; // родитель на красный
    }
    ...
    private class Node { 
        T value;
        Color color; 
        Node left; // левая нода
        Node right; // правая нода

        Node() {color = Color.Red;} // конструктор. у всех начальных нод цвет всегда красный

        Node(T _value) { // конструктор нода по знaчению
            this.value = _value;
            left = null;
            right = null;
            color = Color.Red; // у всех начальных нод цвет всегда красный
        }
    }
